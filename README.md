...
## Знакомство с promise и fetch, сборка клиентской части
...
В 6 лабораторной работе используется встроенная функция fetch с async/await. Файлы ajax.js и sessionsUrls.js удалены.
...
### 5.1. Главная страница с карточками

![Главная страница](./screenshots/main-page.png)

*Описание: Главная страница с заголовком "Активные сеансы удалённого доступа" и карточками устройств, полученными через fetch запрос.*

### 5.2. Страница деталей сеанса

![Страница деталей](./screenshots/product-page.png)

*Описание: Страница деталей сеанса с информацией об устройстве: название, пользователь, IP-адрес, время сеанса, статус. Присутствует кнопка "Назад" для возврата на главную.*

### 5.3. Вкладка Network с запросами fetch

![Network запросы](./screenshots/network-fetch.png)

*Описание: Вкладка Network инструментов разработчика показывает запросы fetch к /sessions и /sessions/1. Статус ответа 200, тип запроса - fetch.*

### 5.4. Бэкенд раздает статику на порту 3000

![Бэкенд статика](./screenshots/backend-static.png)

*Описание: Браузер открыт по адресу http://localhost:3000. Фронтенд загружается с бэкенда, API запросы идут на тот же порт. Отсутствие CORS ошибок в консоли.*
...
**Что удалено по сравнению с 5 лабораторной:**
- Папка modules/ (ajax.js, sessionsUrls.js)

**Что добавлено:**
- Файл vite.config.js
- Папка public/ в бэкенде (результат сборки)
- Скрипты dev, build, preview в package.json
- Раздача статики в index.js бэкенда
...
1. Произведена замена устаревшего механизма XMLHttpRequest на современный fetch с использованием промисов и async/await.
