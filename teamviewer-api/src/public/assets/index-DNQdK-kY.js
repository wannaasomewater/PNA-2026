(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function e(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(r){if(r.ep)return;r.ep=!0;const i=e(r);fetch(r.href,i)}})();class a{constructor(t){this.parent=t}getHTML(t){return`
            ...
            `}addListeners(t,e){document.getElementById(`click-card-${t.id}`).addEventListener("click",e)}render(t,e){const s=this.getHTML(t);this.parent.insertAdjacentHTML("beforeend",s),this.addListeners(t,e)}}class d{constructor(t){this.parent=t}getHTML(t){return`
            ...
            `}render(t){const e=this.getHTML();this.parent.insertAdjacentHTML("beforeend",e),this.addListeners(t)}}class u{constructor(t,e){this.parent=t,this.id=e,this.session=null}get pageRoot(){return document.getElementById("product-page")}getHTML(){return'<div id="product-page"></div>'}showDebugMessage(t,e=!1){const s=this.pageRoot;if(s){if(!document.getElementById("product-debug-info")&&!e)return;e&&(s.innerHTML+=`<div id="product-debug-info" class="alert alert-danger mx-4">${t}</div>`)}}async getData(){try{const t=await fetch(`/sessions/${this.id}`);if(!t.ok)throw t.status===404?new Error("Сеанс не найден"):new Error(`HTTP ошибка: ${t.status}`);this.session=await t.json(),this.renderProduct()}catch(t){console.error("Ошибка загрузки:",t);const e=this.pageRoot;e&&(e.innerHTML+=`<div class="alert alert-danger">${t.message}</div>`)}}renderProduct(){if(!this.session)return;const t={id:this.session.id,src:this.getImageForStatus(this.session.status),title:this.session.deviceName,text:`
        ...
        `}showDebugMessage(t,e=!1){const s=document.getElementById("debug-info");s&&(s.style.display="block",s.className=e?"alert alert-danger mx-4":"alert alert-info mx-4",s.innerHTML=t)}async getData(){this.showDebugMessage("🔄 Загрузка данных с сервера (fetch)...");try{const t="/sessions";this.showDebugMessage(`📡 Запрос к: ${t}`);const e=await fetch(t);if(this.showDebugMessage(`📦 Статус ответа: ${e.status}`),!e.ok)throw new Error(`HTTP ошибка: ${e.status}`);const s=await e.json();this.showDebugMessage(`✅ Получено ${s.length} сеансов`),this.sessions=s,this.renderCards()}catch(t){const e=`❌ Ошибка: ${t.message}`;this.showDebugMessage(e,!0),console.error("Ошибка загрузки:",t)}}renderCards(){const t=document.getElementById("cards-container");if(t){if(t.innerHTML="",this.sessions.length===0){t.innerHTML='<div class="alert alert-warning">Нет активных сеансов</div>';return}this.sessions.forEach(e=>{const s={id:e.id,src:this.getImageForStatus(e.status),title:e.deviceName,text:`Пользователь: ${e.user} | Время: ${e.sessionTime} | IP: ${e.ipAddress}`};new a(t).render(s,this.clickCard.bind(this))})}}getImageForStatus(t){switch(t){case"connected":return"https://img.freepik.com/free-vector/remote-access-concept-illustration_114360-1213.jpg";case"waiting":return"https://img.freepik.com/free-vector/customer-support-concept-illustration_114360-6886.jpg";default:return"https://img.freepik.com/free-vector/teamwork-concept-illustration_114360-678.jpg"}}clickCard(t){const e=t.target.dataset.id;new u(this.parent,e).render()}render(){this.parent.innerHTML="";const t=this.getHTML();this.parent.insertAdjacentHTML("beforeend",t),this.getData()}}const g=document.getElementById("root"),h=new c(g);h.render();
